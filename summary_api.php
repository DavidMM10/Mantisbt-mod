...
// Función para obtener primer KPI
// Número de bugs por líneas de código

function summary_kpi( array $p_projects = array(), $p_level = 0, array $p_cache = null, 
array $p_filter = null ) {
	$t_project_id = helper_get_current_project();
	...
	if( null === $p_cache ) {
		$t_query = new DBQuery();		
		$t_sql = 'SELECT project_id, status, COUNT( status ) AS bugcount FROM {bug}';
		$t_query2 = new DBQuery();
		$t_loc = 'SELECT sequence FROM mantis_custom_field_project_table';		
		if( !empty( $p_filter ) ) {
			$t_subquery = filter_cache_subquery( $p_filter );
			$t_sql .= ' WHERE {bug}.id IN :filter';		
			$t_query->bind( 'filter', $t_subquery );			
		...
		if( !empty( $p_filter ) ) {
			$t_subquery = filter_cache_subquery( $p_filter );			
			$t_loc .= ' WHERE project_id = '.$t_project_id;				
			$t_query2->bind( 'filter', $t_subquery );
		}
		$t_query2->sql( $t_loc );

//OBTENER VALOR LOC DE LA BASE		 
		 $t_total_loc = 0;
		 while( $t_row = $t_query2->fetch() ) {
		  	$t_total_loc = $t_row['sequence'];
		 }		
	}
	...
	foreach( $p_projects as $t_project ) {
		...
		if($t_total_loc == NULL || $t_total_loc == 0){
			$t_total_loc = 1;
		}
		
		$t_bugs_result_kpi = ($t_bugs_total * 1000) / $t_total_loc;
		
# FILTER_PROPERTY_PROJECT_ID filter by project does not work ??
#		$t_bug_link = '<a class="subtle" href="' . config_get( 'bug_count_hyperlink_prefix' ) . '&amp;' . FILTER_PROPERTY_PROJECT_ID . '=' . urlencode( $t_project );
#		summary_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );

function summary_helper_print_kpi( $p_label, $p_total, $p_loc) {
	echo '<tr>';
	printf( '<td class="width50">%s</td>', $p_label );
	printf( '<td class="width12 align-right">%s</td>', $p_total );
	printf( '<td class="width12 align-right">%s</td>', $p_loc );	
}
		summary_helper_print_kpi( string_display_line( $t_name ), $t_bugs_total, $t_total_loc, round($t_bugs_result_kpi) );
		...
		if( count( project_hierarchy_get_subprojects( $t_project ) ) > 0 ) {
			...
			if( count( $t_subprojects ) > 0 ) {
				summary_kpi( $t_subprojects, $p_level + 1, $p_cache );
			}
		}
	}	

	if($t_bugs_result_kpi >= 0 and $t_bugs_result_kpi <= 15){
		printf( '<td style="background-color: #caffcb" class="width8 align-right">%s</td>', round($t_bugs_result_kpi) );
	}else if($t_bugs_result_kpi > 15 and $t_bugs_result_kpi <= 25){
		printf( '<td style="background-color: #fffc53" class="width8 align-right">%s</td>', round($t_bugs_result_kpi) );
	}else{
		printf( '<td style="background-color: #FF9E82  " class="width8 align-right">%s</td>', round($t_bugs_result_kpi) );
	}
	echo '</tr>';
}

// Función para obtener segundo KPI
// Número de reproceso en solución de defectos identificados en certificación

function summary_kpi2( array $p_projects = array(), $p_level = 0, array $p_cache = null, 
array $p_filter = null ) {
	$t_project_id = helper_get_current_project();
	...
	if( null === $p_cache ) {
		$t_query = new DBQuery();
		$t_sql = 'SELECT project_id, (SUBSTRING_INDEX(fixed_in_version, " ", -1) * COUNT( status)) as fixed_in_version, status, COUNT( status ) AS bugcount FROM {bug}';
		if( !empty( $p_filter ) ) {
			$t_subquery = filter_cache_subquery( $p_filter );
			$t_sql .= ' WHERE {bug}.id IN :filter or {bug}.id = 90';
			$t_query->bind( 'filter', $t_subquery );
		}
		$t_sql .= ' GROUP BY project_id, status, fixed_in_version';
		$t_query->sql( $t_sql );		
		$p_cache = array();
		$t_bugs_total_count = 0;
		$t_cert = 0;
		$t_total_reprocesos = 0;
		
		while( $t_row = $t_query->fetch() ) {
			$t_project_id = $t_row['project_id'];
			$t_status = $t_row['status'];
			$t_bugcount = $t_row['bugcount'];
			$t_cert = $t_row['fixed_in_version'];
			$t_bugs_total_count += $t_bugcount;
			summary_helper_build_bugcount( $p_cache, $t_project_id, $t_status, $t_bugcount );
			$t_total_cert = $t_cert;
			
			if($t_total_cert == NULL){
				$t_total_cert = 0;				
			}
			
			$t_total_reprocesos += $t_total_cert;			
		}
		$p_cache["_bugs_total_count_"] = $t_bugs_total_count;		
	}
	...
	foreach( $p_projects as $t_project ) {
		...
		if($t_total_reprocesos == NULL){
			$t_total_reprocesos = 0;
		}

		$t_bugs_result_kpi2 = $t_total_reprocesos / $t_bugs_total;	

		function summary_helper_print_kpi2( $p_label, $p_total, $p_total_cert, $p_result) {
			echo '<tr>';
			printf( '<td class="width50">%s</td>', $p_label );
			printf( '<td class="width12 align-right">%s</td>', $p_total );
			printf( '<td class="width12 align-right">%s</td>', $p_total_cert );	
...
#		summary_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );
		...
		summary_helper_print_kpi2( string_display_line( $t_name ), $t_bugs_total, $t_total_reprocesos, round($t_bugs_result_kpi2, 2) );
				
		if( count( project_hierarchy_get_subprojects( $t_project ) ) > 0 ) {
			$t_subprojects = current_user_get_accessible_subprojects( $t_project );

			if( count( $t_subprojects ) > 0 ) {
				summary_kpi2( $t_subprojects, $p_level + 1, $p_cache );
			}
		}
	}

	if($t_bugs_result_kpi2 <= 1){
		printf( '<td style="background-color: #caffcb" class=" width8 align-right">%s</td>', round($t_bugs_result_kpi2, 2) );
	}else if($t_bugs_result_kpi2 > 1 and $t_bugs_result_kpi2 <= 2){
		printf( '<td style="background-color: #fffc53" class=" width8 align-right">%s</td>', round($t_bugs_result_kpi2, 2) );
	}else{
		printf( '<td style="background-color: #FF9E82  " class=" width8 align-right">%s</td>', round($t_bugs_result_kpi2, 2) );
	}
}


// Función para obtener la cobertura de bugs


function summary_cobertura_bugs( array $p_projects = array(), $p_level = 0, array $p_cache = null, 
array $p_filter = null ) {
	$t_project_id = helper_get_current_project();
	...
	foreach( $p_projects as $t_project ) {
		...
		$t_cobertura_bugs = ((float)$t_bugs_closed * 100) / $t_bugs_total;
		
		function summary_helper_print_cobertura_bugs( $p_label, $p_closed, $p_total, $p_result) {
			echo '<tr>';
			printf( '<td class="width50">%s</td>', $p_label );
			printf( '<td class="width12 align-right">%s</td>', $p_closed );
			printf( '<td class="width12 align-right">%s</td>', $p_total );
...
#		summary_helper_build_buglinks( $t_bug_link, $t_bugs_open, $t_bugs_resolved, $t_bugs_closed, $t_bugs_total );
		...
		summary_helper_print_cobertura_bugs( string_display_line( $t_name ), $t_bugs_closed, $t_bugs_total, $t_cobertura_bugs );

		if( count( project_hierarchy_get_subprojects( $t_project ) ) > 0 ) {
			$t_subprojects = current_user_get_accessible_subprojects( $t_project );

			if( count( $t_subprojects ) > 0 ) {
				summary_cobertura_bugs( $t_subprojects, $p_level + 1, $p_cache );
			}
		}

	}

	if($t_cobertura_bugs <= 100 and $t_cobertura_bugs > 90){
		printf( '<td style="background-color: #caffcb" class=" width8 align-right">%s</td>', round($t_cobertura_bugs, 2)."%");
	}elseif($t_cobertura_bugs <= 90 and $t_cobertura_bugs >= 75){
		printf( '<td style="background-color: #fffc53" class=" width8 align-right">%s</td>', round($t_cobertura_bugs, 2)."%");
	}else{
		printf( '<td style="background-color: #FF9E82  " class=" width8 align-right">%s</td>', round($t_cobertura_bugs, 2)."%");
	}
}
